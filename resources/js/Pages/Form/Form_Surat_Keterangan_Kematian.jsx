...
export default function Form_Surat_Keterangan_Kematian(props) {
    ...
    const [jam_meninggal, set_jam_meninggal] = React.useState(
        dayjs(new Date()),
    );
    ...
    const [identitas, set_identitas] = useState({
        ...
        hari_meninggal: "",
        jam_meninggal:
            (JSON.stringify(jam_meninggal.$H).length == 1
                ? "0" + jam_meninggal.$H
                : jam_meninggal.$H) +
            ":" +
            (JSON.stringify(jam_meninggal.$m).length == 1
                ? "0" + jam_meninggal.$m
                : jam_meninggal.$m),
    ...
    const oc_identitas = (e) => {
        if (e.$H != null) {
            ...
            set_identitas({ ...identitas, ["alamat_kecamatan"]: option });
            ...
            let el = e.target.childNodes[index];
            let option = el.getAttribute("id");
            set_identitas({ ...identitas, ["alamat_kelurahan"]: option });
    ...
    useEffect(() => {
        ...
        axios
            .post(window.location.origin + "/ref_form_surat_keterangan_kematian", {
                id_form: props.id,
            })
            .then(function (response) {
                ...
                const noSurat = parseJSON(row.no_surat, ["", "", ""]);
                ...
                const jamMeninggal = row.jam_meninggal || "";
                if (jamMeninggal.includes(":")) {
                    const [jam, menit] = jamMeninggal.split(":");
                    set_jam_meninggal(dayjs().hour(Number(jam)).minute(Number(menit)));
                }

                const namaHari = ["Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];
                set_identitas((prev) => ({
                    ...prev,
                    nik: row.pasien?.nik || row.nik || "",
                    nama: row.nama || row.pasien?.nama || "",
                    tempat_lahir: row.tempat_lahir || "",
                    tgl_lahir: row.tgl_lahir || row.pasien?.tgl_lahir || "",
                    umur: row.tgl_lahir ? hit_umur(row.tgl_lahir) : "",
                    jenis_kelamin: row.jenis_kelamin || "L",
                    agama: row.agama || "",
                    alamat: row.alamat || row.pasien?.alamat || "",
                    alamat_kelurahan: row.alamat_kelurahan || row.pasien?.alamat_kelurahan || "",
                    alamat_kecamatan: row.alamat_kecamatan || row.pasien?.alamat_kecamatan || "",
                    tgl_meninggal: row.tgl_meninggal || "",
                    hari_meninggal: row.tgl_meninggal ? namaHari[new Date(row.tgl_meninggal).getDay()] : "",
                    jam_meninggal: row.jam_meninggal || prev.jam_meninggal,
                    tgl_surat: row.tgl_surat || "",
                    nama_ttd_dokter: row.nama_ttd_dokter || "",
                }));

                if (row.ttd_dokter && ref_ttd_dokter_ambulan.current) {
                    setTimeout(() => {
                        if (ref_ttd_dokter_ambulan.current?.fromDataURL) {
                            ref_ttd_dokter_ambulan.current.fromDataURL(row.ttd_dokter);
                        }
                    }, 500);
                }
            });
    ...
    const oc_simpan = (e) => {
        ...
        const endpoint = props.id
            ? window.location.origin + "/form_surat_keterangan_kematian/perbarui"
            : window.location.origin + "/form_surat_keterangan_kematian/simpan";
        ...
        axios.post(endpoint, {
            ...
            ttd_dokter: ref_ttd_dokter_ambulan.current && !ref_ttd_dokter_ambulan.current.isEmpty()
                ? ref_ttd_dokter_ambulan.current.getCanvas().toDataURL("image/png")
                : "",
        }).then(response => {
            toast.success(props.id ? "berhasil diperbarui" : "berhasil simpan", { position: toast.POSITION.TOP_RIGHT });
        }).catch(err => {
    ...
    return (
        <div className="min-h-screen bg-slate-200 py-10 print:bg-white print:py-0 w-full font-sans text-black">
            ...
            >
                <HeaderFormSurat />
                <div className="flex justify-center font-bold text-[20px] underline mt-3">
                    ...
                    <div className="w-[20px] h-[20px]">
                        {isPrinting == false && (
                            <input
                                ...
                                name="no_1"
                                type="text"
                                onChange={oc_nomor_surat}
                                value={nomor_surat.no_1}
                            ></input>
                        )}
                        {isPrinting && <div>{nomor_surat.no_1}</div>}
                    </div>
                    <div>/ ADM</div>
                    ...
                    <div className="w-[20px] h-[20px]">
                        {isPrinting == false && (
                            <input
                                ...
                                type="text"
                                onChange={oc_nomor_surat}
                                value={nomor_surat.no_2}
                            ></input>
                        )}
                        {isPrinting && <div>{nomor_surat.no_2}</div>}
                    </div>
                    ...
                    <div className="w-[20px] h-[20px]">
                        {isPrinting == false && (
                            <input
                                ...
                                type="text"
                                onChange={oc_nomor_surat}
                                value={nomor_surat.no_3}
                            ></input>
                        )}
                        {isPrinting && <div>{nomor_surat.no_3}</div>}
                    </div>
                </div>
                <div className="ml-5 mr-5">
                    <div>
                        Yang bertanda tangan dibawah ini, Dokter Ambulance Hebat
                        Dinas Kesehatan Kota Semarang menerangkan dengan
                        sebenarnya bahwa :
                    </div>
                    <div className="ml-3">
                        <div className="flex">
                            ...
                            <div className="w-[75%]">
                                ...
                                {isPrinting && <div>{identitas.nama}</div>}
                            </div>
                        </div>
                        <div className="flex">
                            ...
                            <div className="w-[75%]">
                                ...
                                {isPrinting && <div>{identitas.nik}</div>}
                            </div>
                        </div>
                        <div className="flex">
                            <div className="w-[25%]">
                            ...
                            {isPrinting && <div>{identitas.tempat_lahir}</div>}
                            {isPrinting == false && (
                                <div className="w-[20%]">
                                    <input
                                        type="date"
                        ...
                        <div className="flex">
                            ...
                            <div className="w-[75%]">
                                ...
                                {isPrinting && <div>{identitas.agama}</div>}
                            </div>
                        </div>
                        <div className="flex">
                            ...
                            {isPrinting == false && (
                                <div className="w-[20%]">
                                    ...
                                    >
                                        ...
                                        {get_semua_kecamatan.map((opts, i) => (
                                            <option
                                                key={i}
                                                id={opts.kode_kecamatan}
                                                value={opts.nama_kecamatan}
                                            >
                                                {opts.nama_kecamatan}
                                            </option>
                                        ))}
                            ...
                            {isPrinting == false && (
                                <div className="w-[20%]">
                                    ...
                                    >
                                        ...
                                        {get_semua_kelurahan.map((opts, i) => (
                                            <option
                                                key={i}
                                                id={opts.kode_kelurahan}
                                                value={opts.nama_kelurahan}
                                            >
                                                {opts.nama_kelurahan}
                                            </option>
                                        ))}
                    ...
                    <div className="mt-3">Telah meninggal dunia pada :</div>
                    <div className="ml-3">
                        <div className="flex">
                            ...
                            <div className="w-[68%]">
                                {isPrinting == false && (
                                    <input
                                        ...
                                        name="tgl_meninggal"
                                        onChange={oc_identitas}
                                        value={identitas.tgl_meninggal}
                                    ></input>
                                )}
                                {isPrinting && (
                                    <div>
                                        {identitas.tgl_meninggal.slice(8, 10) +
                                            "/" +
                                            identitas.tgl_meninggal.slice(
                                                5,
                                                7,
                                            ) +
                                            "/" +
                                            identitas.tgl_meninggal.slice(0, 4)}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="mt-3">
                        ...
                        sebenarnya-benarnya dan digunakan sebagaimana mestinya.
                    </div>
                        ...
                        <div>
                            {isPrinting == false && (
                                <input
                                    ...
                                    name="tgl_surat"
                                    onChange={oc_identitas}
                                    value={identitas.tgl_surat}
                                ></input>
                            )}
                            {isPrinting && (
                                <div>
                                    {identitas.tgl_surat.slice(8, 10) +
                                        "/" +
                                        identitas.tgl_surat.slice(5, 7) +
                                        "/" +
                                        identitas.tgl_surat.slice(0, 4)}
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            <div>
                                {isPrinting == false && (
                                    <input
                                        type="text"
                                        className="w-full p-0"
                                        name="nama_ttd_dokter"
                                        onChange={oc_identitas}
                                        value={identitas.nama_ttd_dokter}
                                    ></input>
                                )}
                                {isPrinting && (
                                    <div className="flex justify-center">
                                        {identitas.nama_ttd_dokter}
                                    </div>
                                )}
                            </div>
                            {isPrinting == false && (
