...
export default function Form_Surat_Persetujuan_Tindakan_Medis(props) {
    ...
    const [
        get_nama_kecamatan_yg_bertanda_tangan,
        set_nama_kecamatan_yg_bertanda_tangan,
    ] = useState("-");
    const [
        get_nama_kelurahan_yg_bertanda_tangan,
        set_nama_kelurahan_yg_bertanda_tangan,
    ] = useState("-");
    ...
    useEffect(() => {
        ...
        axios.post(window.location.origin + "/ref_kelurahan", { kode_kecamatan: [] })
            .then(function (response) {
        ...
        if (props.id) {
            axios.post(window.location.origin + "/ref_form_surat_persetujuan_tindakan_medis", { id_form: props.id })
                .then(function (response) {
                    if (response.data) {
                        ...
                        set_yg_bertanda_tangan({
                            ...
                            jenis_kelamin: response.data.jenis_kelamin || "L",
                            alamat: response.data.alamat || "",
                            alamat_kelurahan: response.data.alamat_kelurahan || "-",
                            alamat_kecamatan: response.data.alamat_kecamatan || "",
                        });

                        set_nama_kecamatan_yg_bertanda_tangan(response.data.kecamatan?.nama_kecamatan || "-");
                        set_nama_kelurahan_yg_bertanda_tangan(response.data.kelurahan?.nama_kelurahan || "-");

                        if (response.data.alamat_kecamatan && response.data.alamat_kecamatan !== "-") {
                            axios.post(window.location.origin + "/ref_kelurahan", { kode_kecamatan: response.data.alamat_kecamatan })
                                .then(function (res) { set_semua_kelurahan(res.data); });
                        }

                        set_status_surat(response.data.status_surat || "PERSETUJUAN");

                        set_yg_telah_memberikan({
                            tindakan_medis: response.data.tindakan_medis || "",
                            terhadap: response.data.terhadap || "diri saya sendiri",
                            nama: response.data.memberikan_nama || "",
                            umur: response.data.memberikan_umur || "",
                            jenis_kelamin: response.data.memberikan_jenis_kelamin || "L",
                            alamat: response.data.memberikan_alamat || "",
                            alamat_kelurahan: response.data.memberikan_alamat_kelurahan || "-",
                            alamat_kecamatan: response.data.memberikan_alamat_kecamatan || "",
                        });

                        set_nama_kecamatan_yg_telah_memberikan(response.data.memberikan_kecamatan?.nama_kecamatan || "-");
                        set_nama_kelurahan_yg_telah_memberikan(response.data.memberikan_kelurahan?.nama_kelurahan || "-");

                        if (response.data.memberikan_alamat_kecamatan && response.data.memberikan_alamat_kecamatan !== "-") {
                            axios.post(window.location.origin + "/ref_kelurahan", { kode_kecamatan: response.data.memberikan_alamat_kecamatan })
                                .then(function (res) { set_semua_kelurahan_yg_telah_memberikan(res.data); });
                        }

                        set_tambah_pernyataan(parseJSON(response.data.tambahan_pernyataan, []));
                        if (response.data.tgl_surat) {
                            set_tgl_surat(response.data.tgl_surat);
                        }

                        const nama_saksi = parseJSON(response.data.nama_saksi, []);
                        if (nama_saksi.length > 0) {
                            set_nama_ttd_saksi(nama_saksi[0] || "");
                            if (nama_saksi.length > 1) {
                                set_nama_tambah_saksi(nama_saksi.slice(1));
                            }
                        }

                        set_ttd_dokter_paramedis(response.data.status_ttd_dokter_paramedis || "Dokter");
                        set_nama_ttd_dokter_paramedis(response.data.nama_dokter_paramedis || "");


                        // TIMPA KUMPULAN SET TIMEOUT TTD LAMA MENJADI INI:
                        if (response.data.ttd_yang_membuat_pernyataan && ref_ttd_yang_membuat_pernyataan.current) {
                            setTimeout(() => {
                                const canvas = ref_ttd_yang_membuat_pernyataan.current.getCanvas();
                                ref_ttd_yang_membuat_pernyataan.current.clear();
                                ref_ttd_yang_membuat_pernyataan.current.fromDataURL(
                                    response.data.ttd_yang_membuat_pernyataan,
                                    { width: canvas.offsetWidth, height: canvas.offsetHeight }
                                );
                            }, 1500);
                        }

                        if (response.data.ttd_dokter_paramedis && ref_ttd_dokter_paramedis.current) {
                            setTimeout(() => {
                                const canvas = ref_ttd_dokter_paramedis.current.getCanvas();
                                ref_ttd_dokter_paramedis.current.clear();
                                ref_ttd_dokter_paramedis.current.fromDataURL(
                                    response.data.ttd_dokter_paramedis,
                                    { width: canvas.offsetWidth, height: canvas.offsetHeight }
                                );
                            }, 1500);
                        }

                        const ttd_saksi_arr = parseJSON(response.data.ttd_saksi, []);
                        if (ttd_saksi_arr.length > 0) {
                            setTimeout(() => {
                                if (ttd_saksi_arr[0] && ref_ttd_saksi.current) {
                                    const canvas0 = ref_ttd_saksi.current.getCanvas();
                                    ref_ttd_saksi.current.clear();
                                    ref_ttd_saksi.current.fromDataURL(
                                        ttd_saksi_arr[0],
                                        { width: canvas0.offsetWidth, height: canvas0.offsetHeight }
                                    );
                                }
                                if (ttd_saksi_arr.length > 1) {
                                    ttd_saksi_arr.slice(1).forEach((ttd, i) => {
                                        if (ttd && ref_ttd_tambah_saksi.current[i]) {
                                            const canvasX = ref_ttd_tambah_saksi.current[i].getCanvas();
                                            ref_ttd_tambah_saksi.current[i].clear();
                                            ref_ttd_tambah_saksi.current[i].fromDataURL(
                                                ttd,
                                                { width: canvasX.offsetWidth, height: canvasX.offsetHeight }
                                            );
                                        }
                                    });
                                }
                            }, 1800); // Diperpanjang agar saksi tambahan punya waktu mount sempurna
                        }
                    }
                });
        }
    ...
    const [
        get_nama_kecamatan_yg_telah_memberikan,
        set_nama_kecamatan_yg_telah_memberikan,
    ] = useState("-");
    const [
        get_nama_kelurahan_yg_telah_memberikan,
        set_nama_kelurahan_yg_telah_memberikan,
    ] = useState("-");
    const [
        get_semua_kecamatan_yg_telah_memberikan,
        set_semua_kecamatan_yg_telah_memberikan,
    ] = useState([]);
    const [
        get_semua_kelurahan_yg_telah_memberikan,
        set_semua_kelurahan_yg_telah_memberikan,
    ] = useState([]);
    ...
    const oc_tambah_pernyataan = (i) => {
        ...
        ref_ttd_tambah_saksi.current = [get_tambah_pernyataan].map(
            (ref, index) =>
                (ref_ttd_tambah_saksi.current[index] = React.createRef()),
        );
    };
    ...
    const [get_ttd_dokter_paramedis, set_ttd_dokter_paramedis] =
        useState("Dokter");
    const [get_nama_ttd_dokter_paramedis, set_nama_ttd_dokter_paramedis] =
        useState("");
    ...
    const oc_simpan = (e) => {
        ...
        const arrSaksiSigs = [];
        if (ref_ttd_saksi.current && !ref_ttd_saksi.current.isEmpty()) {
            arrSaksiSigs.push(ref_ttd_saksi.current.getCanvas().toDataURL('image/png'));
        } else {
            arrSaksiSigs.push("");
        }

        ref_ttd_tambah_saksi.current.forEach(ref => {
            if (ref && !ref.isEmpty()) {
                arrSaksiSigs.push(ref.getCanvas().toDataURL('image/png'));
            } else {
                arrSaksiSigs.push("");
            }
        });
        ...
        const endpoint = props.id 
            ? window.location.origin + "/form_surat_persetujuan_tindakan_medis/perbarui" 
            : window.location.origin + "/form_surat_persetujuan_tindakan_medis/tambah";
        ...
        axios
            .post(
                endpoint,
                {
                    ...
                    yg_bertanda_tangan_jenis_kelamin:
                        get_yg_bertanda_tangan.jenis_kelamin,
                    yg_bertanda_tangan_alamat: get_yg_bertanda_tangan.alamat,
                    yg_bertanda_tangan_alamat_kelurahan:
                        get_yg_bertanda_tangan.alamat_kelurahan,
                    yg_bertanda_tangan_alamat_kecamatan:
                        get_yg_bertanda_tangan.alamat_kecamatan,

                    status_surat: get_status_surat,
                    yg_telah_memberikan_tindakan_medis:
                        get_yg_telah_memberikan.tindakan_medis,
                    yg_telah_memberikan_terhadap:
                        get_yg_telah_memberikan.terhadap,
                    yg_telah_memberikan_nama: get_yg_telah_memberikan.nama,
                    yg_telah_memberikan_umur: get_yg_telah_memberikan.umur,
                    yg_telah_memberikan_jenis_kelamin:
                        get_yg_telah_memberikan.jenis_kelamin,
                    yg_telah_memberikan_alamat: get_yg_telah_memberikan.alamat,
                    yg_telah_memberikan_alamat_kelurahan:
                        get_yg_telah_memberikan.alamat_kelurahan,
                    yg_telah_memberikan_alamat_kecamatan:
                        get_yg_telah_memberikan.alamat_kecamatan,
                    ...
                    ttd_yang_membuat_pernyataan: ref_ttd_yang_membuat_pernyataan.current && !ref_ttd_yang_membuat_pernyataan.current.isEmpty() ? ref_ttd_yang_membuat_pernyataan.current.getCanvas().toDataURL('image/png') : "",
                    ttd_dokter_paramedis: ref_ttd_dokter_paramedis.current && !ref_ttd_dokter_paramedis.current.isEmpty() ? ref_ttd_dokter_paramedis.current.getCanvas().toDataURL('image/png') : "",
                    ttd_saksi: arrSaksiSigs,
                },
            )
            .then(function (response) {
        ...
        <div className="min-h-screen bg-slate-200 py-10 print:bg-white print:py-0 w-full font-sans text-black">
            ...
            >
                        ...
                        <div className="grid col-start-2 col-end-6">
                            {isPrinting == false && (
                                <input
                                    ...
                                    type="text"
                                    name="nama"
                                    value={get_yg_bertanda_tangan.nama}
                            ...
                            <div className="flex">
                                {isPrinting == false && (
                                    <input
                                        ...
                                        type="text"
                                        name="umur"
                                        value={get_yg_bertanda_tangan.umur}
                                        onChange={oc_yg_bertanda_tangan}
                                    ></input>
                                )}
                                {isPrinting && (
                                ...
                                <div className="flex flex-wrap">
                                    <div className="w-[30%]">
                                        ...
                                        >
                                            ...
                                            {get_semua_kecamatan.map(
                                                (opts, i) => (
                                                    <option
                                                        key={i}
                                                        id={opts.kode_kecamatan}
                                                        value={
                                                            opts.nama_kecamatan
                                                        }
                                                    >
                                    ...
                                    <div className="w-[30%]">
                                        ...
                                        >
                                            ...
                                            {get_semua_kelurahan.map(
                                                (opts, i) => (
                                                    <option
                                                        key={i}
                                                        id={opts.kode_kelurahan}
                                                        value={
                                                            opts.nama_kelurahan
                                                        }
                                                    >
                ...
                <div className="flex justify-center font-bold mt-3 mb-3">
                    ...
                    {isPrinting && <div>{get_status_surat}</div>}
                </div>
                <div className="ml-5 mr-5">
                    <div className="flex">
                        <div className="w-[25%]">
                    ...
                    <div className="mb-3 flex">
                        ...
                        <div>
                            {isPrinting == false && (
                                ...
                                >
                                    <option value="diri saya sendiri">
                                        diri saya sendiri
                                    </option>
                        ...
                        <div className="grid col-start-2 col-end-6">
                            {isPrinting == false && (
                                <input
                                    ...
                                    name="nama"
                                    onChange={oc_yg_telah_memberikan}
                                    value={get_yg_telah_memberikan.nama}
                                ></input>
                            )}
                            {isPrinting && (
                                <div>{get_yg_telah_memberikan.nama}</div>
                            )}
                        </div>
                    </div>
                    <div className="grid grid-cols-5">
                            ...
                            <div className="flex">
                                {isPrinting == false && (
                                    <input
                                        ...
                                        name="umur"
                                        onChange={oc_yg_telah_memberikan}
                                        value={get_yg_telah_memberikan.umur}
                                    ></input>
                                )}
                                {isPrinting && (
                                    <div>{get_yg_telah_memberikan.umur}</div>
                                )}
                                <div className="ml-1 mr-2">Tahun</div>
                                <div>/ Jenis Kelamin:</div>
                                {isPrinting == false && (
                            ...
                            {isPrinting == false && (
                                <div className="flex flex-wrap">
                                    <div className="w-[30%]">
                                        ...
                                        >
                                            <option value="-">Kecamatan</option>
                                            {get_semua_kecamatan_yg_telah_memberikan.map(
                                                (opts, i) => (
                                                    <option
                                                        key={i}
                                                        id={opts.kode_kecamatan}
                                    ...
                                    <div className="w-[30%]">
                                        ...
                                        >
                                            <option value="-">Kelurahan</option>
                                            {get_semua_kelurahan_yg_telah_memberikan.map(
                                                (opts, i) => (
                                                    <option
                                                        key={i}
                                                        id={opts.kode_kelurahan}
                                    ...
                                    <div className="w-[40%]">
                                        <input
                                            ...
                                            onChange={oc_yg_telah_memberikan}
                                            value={
                                                get_yg_telah_memberikan.alamat
                                            }
                                        ></input>
                                    </div>
                                </div>
                    ...
                    <div className="mt-3 mb-3">
                        Telah menyatakan dengan sesungguhnya tanpa paksaan bahwa
                        :
                    </div>
                    <div>
                        <div className="flex">
                            ...
                            <div className="w-[95%]">
                                Bersedia mengikuti SOP perujukan menggunakan
                                Ambulance Hebat (tidak bisa memilih RS yang
                                dikehendaki).
                            </div>
                        </div>
                        <div className="flex">
                            ...
                            <div className="w-[95%]">
                                Telah diberikan informasi dan penjelasan, bahwa
                                penggunaan ambulance transport ini hanya
                                mengantarkan ke RS / tempat yang dituju dan
                                petugas akan langsung pulang setelah
                                mengantarkan.
                            </div>
                        </div>
                        <div className="flex">
                            ...
                            <div className="w-[95%]">
                                Seluruh berkas dan keperluan di RS telah
                                dipersiapkan oleh keluarga pasien dan menjadi
                                tanggung jawab kami (keluarga).
                            </div>
                        </div>
                        <div className="flex">
                            <div className="w-[5%]">d.</div>
                            <div className="w-[95%]">
                                Segala akibat dan resiko yang akan terjadi
                                menjadi tanggung jawab kami (keluarga) dan kami
                                (keluarga) bersedia untuk menunggu dirumah sakit
                                dan tidak akan meminta petugas ambulance untuk
                                menemani menunggu di RS.
                            </div>
                        </div>
                        <div className="flex">
                            <div className="w-[5%]">e.</div>
                            <div className="w-[95%]">
                                Bersedia mengikuti protokol kesehatan yang
                                berlaku di RS meliputi, Skrining Covid-19,
                                tetalaksana penanganan pasien dari awal masuk
                                sampai selesai, ruangan isolasi dan perawatan
                                jenazah Covid-19.
                            </div>
                        </div>
                        <div className="flex">
                            ...
                            <div className="w-[95%]">
                                Bersedia menunggu di IGD sampai batas waktu yang
                                tidak bisa di tentukan.
                            </div>
                        </div>
                        <div className="flex">
                            <div className="w-[5%]">g.</div>
                            <div className="w-[95%]">
                                Telah saya pahami sepenuhnya informasi dan
                                penjelasan yang diberikan dokter / paramedis.
                            </div>
                        </div>
                        {isPrinting == false && (
                                ...
                                return (
                                    <div key={i + 1}>
                                        <div className="flex">
                                            ...
                                            <textarea
                                                className="w-[90%] text-xs md:text-sm sm:text-xs"
                                                type="text"
                                                value={get_tambah_pernyataan[i]}
                                                onChange={(e) =>
                                                    oc_value_tambah_pernyataan(
                                                        e,
                                                        i,
                                                    )
                                                }
                                            ></textarea>
                                            <button
                                                className="w-[5%] text-xs md:text-sm sm:text-xs bg-red-500 hover:bg-red-700 text-white font-bold rounded"
                                                type="button"
                                                onClick={() =>
                                                    oc_hapus_tambah_pernyataan(
                                                        i,
                                                    )
                                                }
                                            >
                    ...
                    <div className="mt-3">
                        Pernyataan ini kami buat dengan penuh kesadaran tanpa
                    ...
                    <div className="grid grid-cols-3 mt-5">
                                    ...
                                    get_nama_tambah_saksi.map((data, i) => {
                                        ...
                                        return (
                                            <div key={i} className="mt-1">
                                                ...
                                                <div className="flex">
                                                    <input
                                                        className="w-5/6 text-xs md:text-sm sm:text-xs"
                                                        type="text"
                                                        value={
                                                            get_nama_tambah_saksi[
                                                                i
                                                            ]
                                                        }
                                                        onChange={(e) =>
                                                            oc_value_tambah_saksi(
                                                                e,
                                                                i,
                                                            )
                                                        }
                                ...
                                <div className="flex justify-center">
                                    {isPrinting == false && (
                                        <input
                                            ...
                                            type="text"
                                            value={
                                                get_nama_ttd_dokter_paramedis
                                            }
                                            onChange={(e) =>
                                                set_nama_ttd_dokter_paramedis(
                                                    e.target.value,
                                                )
                                            }
                                        ></input>
                        ...
                        <div className="flex justify-center">
                            <div>
                                ...
                                <div className="flex justify-center">
                                    {/* <input className="p-0" type="text"></input> */}
                                ...
                                {isPrinting == false && (
                                    <div className="flex justify-center">
                                        <button
                                            type="button"
                                            onClick={
                                                oc_hapus_ttd_yang_membuat_pernyataan
                                            }
                                            className="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none"
                                        >
                                            Hapus
                ...
                {/* END OF WHITE PAPER */}
            {/* --- TOMBOL BAWAH --- */}
            <div className="grid grid-cols-4 mt-3 ml-3 mr-3 text-xs md:text-sm sm:text-xs print:hidden">
                <div></div>
                <button
                    type="button"
                    onClick={oc_simpan}
